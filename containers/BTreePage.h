...
{
       using value_type = KeyType;
       using ObjIDType  = RefType;
       using Comp       = Comparator;
       using Entry      = tagObjectInfo<KeyType, RefType>;
};
...
int binary_search(Container& container, int first, int last, const ObjType &object, Comp &comp)
{
       if( first >= last )
               return first;
       while( first < last )
       {
               ...
               if( !comp(object, midKey) && !comp(midKey, object) )
                       return mid;
